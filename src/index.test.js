...
describe('factory-bem-class', function () {
  ...
  describe('({el: string, modName: boolean})', function () {
    const cn = factoryBemClass(b);

    test(cn({[m[0]]: true}),                       `${b} ${b}--${m[0]}`);
    test(cn({[m[0]]: true, [m[1]]: false}),        `${b} ${b}--${m[0]}`);
    test(cn({[m[0]]: true, [m[1]]: true}),         `${b} ${b}--${m[0]} ${b}--${m[1]}`);
    test(cn({el: e, [m[0]]: true}),                `${b}__${e} ${b}__${e}--${m[0]}`);
    test(cn({el: e, [m[0]]: true, [m[1]]: false}), `${b}__${e} ${b}__${e}--${m[0]}`);
    test(cn({el: e, [m[0]]: true, [m[1]]: true}),  `${b}__${e} ${b}__${e}--${m[0]} ${b}__${e}--${m[1]}`);
  });
});
